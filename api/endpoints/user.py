...
async def create_user(user: User, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def get_users(offset: int = 0, limit: int = 10, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
